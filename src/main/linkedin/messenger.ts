...
export async function sendWelcomeDM(
  ...
  try {
    ...
    // Pause until page is loaded/search results appear (4-5s delay as requested)
    console.log(`[Messenger] Pausing natural 4-5s for search results...`);
    await humanDelay(4000, 5000);

    // Press Enter to submit search if dropdown hasn't caught it yet
    await page.keyboard.press("Enter");
    await humanDelay(2000, 3000);

    // Look for search results and click the first matching profile
    const resultItem = await page.waitForSelector('.msg-conversation-listitem, .msg-conversations-list-item, [data-view-name="message-list-item"]', { timeout: 6000 }).catch(() => null);
...
export async function checkLeadThreadForReply(
  ...
  try {
    ...
    const messages = await page.evaluate(() => {
      const msgs: Array<{sender: string, text: string}> = [];
      const blocks = document.querySelectorAll('.msg-s-message-list__event, .msg-s-message-group, .msg-s-event-listitem');
      blocks.forEach(block => {
        const senderEl =
          block.querySelector('.msg-s-message-group__name') ||
          block.querySelector('.msg-s-message-list-item__user-name') ||
        ...
        const textElements = block.querySelectorAll('.msg-s-event-listitem__body, .msg-s-event__content p, .msg-s-event__content span, p');
        const texts: string[] = [];
        textElements.forEach(el => {
          const t = el.textContent?.trim();
          if (t && t.length > 0) texts.push(t);
    ...
    for (const msg of messages) {
      const senderLower = msg.sender.toLowerCase();
      const role = (senderLower.includes('you') || senderLower.includes(myNamePart)) ? 'assistant' : 'user';
